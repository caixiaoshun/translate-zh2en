...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.position_embed = RopePositionEmbedding(self.head_dim)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_mask):
        ...
        y = self.attn(x, x, x, attn_mask=encoder_mask)
        x = self.norm_1(x + y)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, e_output, encoder_mask, decoder_mask):
        y = self.self_attn(x, x, x, attn_mask=decoder_mask)
        x = self.norm_1(x + y)
        
        y = self.cross_attn(x, e_output, e_output, attn_mask=encoder_mask)
        x = self.norm_2(x + y)
...
class TranslateModelPostNorm(nn.Module):
    def __init__(
        self,
        vocab_size,
        encoder_layers,
